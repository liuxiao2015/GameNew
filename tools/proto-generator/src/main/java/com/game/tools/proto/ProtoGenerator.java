 ...
 * 使用方式:
 * <pre>
 * java -jar proto-generator.jar \
 *     --scan-packages=com.game.api \
 *     --output-dir=services/service-api/src/main/proto \
...
public class ProtoGenerator {
    ...
    private void analyzeClass(Class<?> clazz) {
        ...
        messageInfo.setDesc(annotation.desc());
        messageInfo.setProtocolId(annotation.protocolId());
        messageInfo.setJavaClass(clazz);
    ...
    private FieldInfo analyzeField(Field field, int number, String desc) {
        ...
        if (List.class.isAssignableFrom(type)) {
            fieldInfo.setRepeated(true);
            if (genericType instanceof ParameterizedType pt) {
                Type actualType = pt.getActualTypeArguments()[0];
                if (actualType instanceof Class<?> actualClass) {
                    fieldInfo.setProtoType(getProtoType(actualClass));
                }
            }
        }
        // 处理 Map 类型
        else if (Map.class.isAssignableFrom(type)) {
            fieldInfo.setMap(true);
            if (genericType instanceof ParameterizedType pt) {
                Type keyType = pt.getActualTypeArguments()[0];
                Type valueType = pt.getActualTypeArguments()[1];
                if (keyType instanceof Class<?> keyClass && valueType instanceof Class<?> valueClass) {
                    fieldInfo.setMapKeyType(getProtoType(keyClass));
                    fieldInfo.setMapValueType(getProtoType(valueClass));
                }
            }
        }
        // 普通类型
        else {
            fieldInfo.setProtoType(getProtoType(type));
        }
    ...
    private void generateProtoFile(String module, List<MessageInfo> messages) throws IOException {
        ...
        // 生成消息
        for (MessageInfo msg : messages) {
            if (!msg.getDesc().isEmpty()) {
                sb.append("// ").append(msg.getDesc()).append("\n");
            }
            ...
            for (FieldInfo field : msg.getFields()) {
                ...
                if (!field.getDesc().isEmpty()) {
                    sb.append("// ").append(field.getDesc()).append("\n    ");
                }
                ...
                if (field.isRepeated()) {
                    ...
                    sb.append("map<").append(field.getMapKeyType())
                      .append(", ").append(field.getMapValueType()).append("> ");
                    sb.append(toSnakeCase(field.getName())).append(" = ").append(field.getNumber()).append(";\n");
                    continue;
                }
                
                sb.append(field.getProtoType()).append(" ");
                sb.append(toSnakeCase(field.getName())).append(" = ").append(field.getNumber()).append(";\n");
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        String outputDir = "services/service-api/src/main/proto/generated";
